...
| `GET /api/members/{key}` | the member, with `birth_date`, `government_name` (the name as Rijksoverheid writes it) and `government_functions`: every post they held in a cabinet since 1945 (`function` as Rijksoverheid names it, `cabinet`, `cabinet_key`, the normalised `post` and `ministry`, `seat`, `acting`, `party` (`short`, `faction`), `from_date`, `to_date`, oldest first; empty for a member who held none). A minister who never sat in parliament has the name Rijksoverheid gives; a bewindspersoon the Tweede Kamer has no person for is a member of their own (`rijksoverheid_<initials>_<surname>`) |
