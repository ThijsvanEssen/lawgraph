...
from lawgraph.core.citations import _ART_NUM_PAT, _ART_NUMS_PAT, _parse_article_nums
...
_ARTICLE_RE = re.compile(
    ...
    rf"(?:(?P<arabic>{_ART_NUMS_PAT})|(?P<letters>{_LETTER_LIST}))(?![\w])(?P<rest>.*)$",
...
_PAREN_ARTICLE_RE = re.compile(
    rf"^\s*artikel(?:en)?\s+(?P<nums>{_ART_NUMS_PAT})(?![\w])(?P<tail>.*)$",
    re.IGNORECASE | re.DOTALL,
)
_ROMAN_ONLY_RE = re.compile(rf"^{_ROMAN_NUM}$")
_ARABIC_ONLY_RE = re.compile(rf"^{_ART_NUM_PAT}$", re.IGNORECASE)
_MAX_RANGE = 100
...
def _article_numbers(match: re.Match[str]) -> list[str]:
    if match["arabic"]:
        return _expand(_parse_article_nums(match["arabic"]))
...
def _law_and_refs(rest: str, own: list[str]) -> tuple[list[dict[str, str]], str | None]:
    ...
    for inner in _PAREN_RE.findall(rest):
        ...
        if article:
            ...
            numbers = _expand(_parse_article_nums(article["nums"]))
