...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Normalize historical BWB toestanden into versioned instrument/article nodes."
    )
    parser.add_argument(
        "--since",
        default=None,
    ...
    try:
        ...
        logger.error("BWB history normalization failed: %s", exc)
        sys.exit(1)

    logger.info("BWB history normalization complete.")
